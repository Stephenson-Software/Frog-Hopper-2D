...
// Frog-Hopper-2D was started, and nothing else.
//
// One event, `startup`, tagged with the version; it carries the program name
// and nothing about the person running it, their machine or the game. It is
...
#ifndef FROG_HOPPER_2D_USAGE_REPORTING_H
#define FROG_HOPPER_2D_USAGE_REPORTING_H
...
static const char *const PROGRAM_NAME = "Frog-Hopper-2D";
...
// copy of the program).
static const char *const KEY = "_HIs3VODmbtYzwsYT6ryy97XTbr4jSyTHk92Uk1PsRM";
...
static const char *const ENV_ENDPOINT = "FROG_HOPPER_2D_USAGE_REPORTING_ENDPOINT";
...
/**
 * Where the settings file lives: %APPDATA%\Frog-Hopper-2D on Windows,
 * ~/Library/Application Support/Frog-Hopper-2D on macOS, and
 * $XDG_CONFIG_HOME/Frog-Hopper-2D (default ~/.config/Frog-Hopper-2D) elsewhere.
...
inline std::string settingsFileContent() {
    return std::string("# ") + PROGRAM_NAME + " usage reporting - "
        ...
        "# the game. Set enabled to false to turn it off. TRACE_USAGE_REPORTING=off\n"
...
inline std::string notice(const std::string &path) {
    ...
    return std::string("Usage reporting is on: ") + PROGRAM_NAME
        + " sends a startup event (its name and version) to https://trace.danielstephenson.dev"
          " - nothing about you, your machine or the game. Turn it off with enabled=false in "
...
#if defined(FROG_HOPPER_VERSION)
    return FROG_HOPPER_VERSION;
#else
